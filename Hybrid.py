...
import numpy as np
import keras
from keras.layers import Conv2D, MaxPooling2D, BatchNormalization, Flatten, Dense, Dropout, LSTM, TimeDistributed, Reshape
...
def prepare_datasets(test_size, validation_size):
    X, y, labels = load_data(DATA_PATH)  # Now it correctly captures labels
    ...
    return X_train, X_validation, X_test, y_train, y_validation, y_test, labels  # Include labels in the return statement
...
def build_model(input_shape):
    ...
    model.add(layers.Conv2D(32, (3, 3), padding='same', kernel_regularizer=regularizers.l2(0.001), input_shape=input_shape))
...
if __name__ == "__main__":
    X_train, X_validation, X_test, y_train, y_validation, y_test, labels = prepare_datasets(0.15,
                                                                                            0.15)  # Adjusted to receive labels
